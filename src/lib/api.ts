...
/* ====================== Error helpers ====================== */
// Fungsi ini sekarang BISA dipakai, tapi kita akan ambil JSON-nya
...
// +++ FUNGSI HELPER BARU UNTUK MENDAPATKAN DATA JSON DARI ERROR +++
async function getErrorData(res: Response) {
  try {
    const ct = res.headers.get('content-type') || '';
    if (ct.includes('application/json')) {
      return await res.json().catch(() => ({ message: 'Failed to parse error JSON' }));
    }
    const text = await res.text();
...
  // ================================================================
  // +++ ⚠️ INI PERBAIKANNYA (BLOK 1) ⚠️ +++
  // ================================================================
  if (!res.ok) {
    // Alih-alih melempar string jelek, kita buat error object
    // yang meniru struktur Axios, karena 'useAuth.tsx' mencarinya.
    const errorData = await getErrorData(res);
    const cleanMessage = errorData?.message || errorData?.error || `HTTP ${res.status}`;
    
    // Buat error baru dengan pesan yang bersih
    const error = new Error(cleanMessage);
    
    // Lampirkan 'response' object agar useAuth.tsx bisa menemukannya
    (error as any).response = {
      data: errorData, // Ini berisi { message: "..." }
      status: res.status,
      config: { url: url }
    };
    
    // Lempar error yang "kaya" ini
    throw error;
  }
  // ================================================================
  // +++ AKHIR PERBAIKAN (BLOK 1) +++
  // ================================================================
...
  // ================================================================
  // +++ ⚠️ INI PERBAIKANNYA (BLOK 2) ⚠️ +++
  // ================================================================
  if (!res.ok) {
    // Lakukan hal yang sama seperti di 'api'
    const errorData = await getErrorData(res);
    const cleanMessage = errorData?.message || errorData?.error || `HTTP ${res.status}`;
    
    const error = new Error(cleanMessage);
