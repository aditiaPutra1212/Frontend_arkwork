...
type CompanyProfile = {
  ...
  socials: Partial<
    Record<
      "website" | "linkedin" | "instagram" | "facebook" | "tiktok" | "youtube",
      string
    >
  >;
};
...
function getPaymentLink(plan?: Plan | null): string | null {
  ...
  if (plan.priceId && MIDTRANS_PAYMENT_LINK_BASE)
    return (
      MIDTRANS_PAYMENT_LINK_BASE.replace(/\/+$/, "") +
      "/" +
      plan.priceId.replace(/^\/+/, "")
    );
  return null;
}
function uuid() {
    ...
    if (window.snap?.pay) {
      await new Promise<void>((resolve, reject) => {
        window.snap!.pay(token, {
          ...
          onClose: () =>
            reject(new Error("Pembayaran ditutup sebelum selesai")),
        });
      });
    } else {
      savePaymentRecord({
              ...
              <label className="block">
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    title={siShowPw ? "Sembunyikan" : "Tampilkan"}
                  >
                    {siShowPw ? (
