...
function findEmailDeep(obj: unknown, depth = 0): string | undefined {
  ...
  if (typeof obj === "string")
    return EMAIL.test(obj.trim()) ? obj.trim() : undefined;
  ...
  if (typeof obj === "object" && obj) {
    ...
    for (const k of Object.keys(rec)) {
      const v = rec[k];
      if (typeof v === "string" && /email/i.test(k) && EMAIL.test(v.trim()))
        return v.trim();
    }
    for (const k of Object.keys(rec)) {
...
export default function Nav() {
  ...
  const employerLinks = useMemo(
    () => [
      {
        href: "/employer/jobs",
        label: t("emp.manageJobs", { defaultMessage: "Manage Jobs" }),
      },
    ],
    [t],
  );
    ...
    (async () => {
      ...
      if (U.email) {
        ...
        try {
          ...
          const legacy = toAbs(
            u?.profile?.photo?.dataUrl as string | undefined,
          );
          if (!cancelled && legacy) {
            setPhotoURL(legacy);
            return;
...
function FileTextIcon(p: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" {...p}>
      ...
      />
      <path
        d="M14 2v6h6M8 9h3M8 13h8M8 17h6"
        stroke="currentColor"
        strokeWidth={2}
        strokeLinecap="round"
      />
...
function LogoutIcon(p: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" {...p}>
      ...
      />
      <path
        d="M4 21h6a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2H4"
        stroke="currentColor"
        strokeWidth={2}
      />
    </svg>
...
function GlobeIcon(p: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" {...p}>
      <circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth={2} />
      <path
        d="M3 12h18M12 3a12 12 0 0 0 0 18M12 3a12 12 0 0 1 0 18"
        stroke="currentColor"
        strokeWidth={2}
        strokeLinecap="round"
      />
...
function SettingsIcon(p: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" {...p}>
      ...
      />
      <path
        d="M19.4 15a7.97 7.97 0 0 0 0-6l-2.1.5a6 6 0 0 0-1.5-1.5l.5-2.1a8 8 0 0 0-6 0l.5 2.1a6 6 0 0 0-1.5 1.5l-2.1-.5a7.97 7.97 0 0 0 0 6l2.1-.5a6 6 0 0 0 1.5 1.5l-.5 2.1a8 8 0 0 0 6 0l-.5-2.1a6 6 0 0 0 1.5-1.5l2.1.5Z"
        stroke="currentColor"
        strokeWidth={2}
      />
    </svg>
